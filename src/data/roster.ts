...
export const ROSTER: CharacterDef[] = [
  {
    ...
    cards: [
      atk({ id: 'volt-jab', name: '스파크 잽', range: both(1), damage: 20, energyCost: 10, fx: 'punch', desc: '앞뒤 한 칸을 동시에 지지는 약공격. 빠르고 저렴하다.' }),
      atk({ id: 'volt-leech', name: '포크 라이트닝', range: FORK, damage: 20, energyCost: 20, drain: 10, fx: 'bolt', desc: '앞뒤 대각선 네 갈래(X자)로 갈라지는 번개. 정면은 사각. 적중 시 상대 기력 10 흡수.' }),
      atk({ id: 'volt-bolt', name: '아크 볼트', range: beamBoth(1, 2), damage: 30, energyCost: 30, fx: 'bolt', desc: '같은 줄 앞뒤 2칸씩 뻗는 전격 빔.' }),
  ...
  },
  {
    ...
    cards: [
      atk({ id: 'titan-hammer', name: '해머 핸드', range: both(1), damage: 30, energyCost: 10, fx: 'punch', desc: '앞뒤 한 칸을 후려치는 강타. 싸고 묵직하다.' }),
  ...
  },
  {
    ...
    cards: [
      atk({ id: 'nova-palm', name: '팜 펄스', range: both(1), damage: 20, energyCost: 10, fx: 'orb', desc: '앞뒤 한 칸을 튕겨내는 견제 펄스.' }),
      atk({ id: 'nova-lance', name: '이온 랜스', range: beam(2, 4), damage: 30, energyCost: 25, pierce: true, fx: 'orb', desc: '앞 2~4칸 관통 광선. 전방 전용 저격 — 상대 보호막을 무시하고, 바로 앞은 사각.' }),
      atk({ id: 'nova-blast', name: '노바 블래스트', range: beam(1, 5), damage: 40, energyCost: 35, fx: 'orb', desc: '같은 줄 끝까지 닿는 최장 구체. 전방 전용 주포.' }),
  ...
  },
  {
    ...
    maxEnergy: 100,
    startEnergy: 50,
    passive: { desc: '데이터 흡수: 공격으로 피해를 주면 체력 +15.', lifesteal: 15 },
    cards: [
      ...
      atk({ id: 'cipher-cross', name: '크로스 슬래시', range: CROSS, damage: 30, energyCost: 15, fx: 'slash', desc: '상·하·좌·우 네 칸을 동시에 베는 십자 범위. 등 뒤도 벤다.' }),
  ...
  },
  {
    ...
    cards: [
      ...
      atk({ id: 'aegis-bash', name: '실드 배시', range: bar(1), damage: 30, energyCost: 30, push: 1, fx: 'shield', desc: '앞 한 칸의 세 줄을 방패로 후려쳐 한 칸 밀어낸다. 전방 전용.' }),
      {
        id: 'aegis-wall',
        name: '아이언 커튼',
        kind: 'guard',
      ...
      atk({ id: 'aegis-drive', name: '벌워크 드라이브', range: [fwd(-1), fwd(1), fwd(2)], damage: 50, energyCost: 55, selfShield: 20, fx: 'shield', signature: true, accent: '#5b7ee0', desc: '시그니처. 같은 줄의 뒤 한 칸 + 앞 두 칸을 쓸어버리는 돌진 — 사용 시 보호막 +20.' }),
    ],
  },
  {
    id: 'ember',
    ...
    cards: [
      ...
      atk({ id: 'ember-blitz', name: '오버히트 블리츠', range: beam(1, 2), damage: 40, energyCost: 20, recoil: 5, fx: 'rush', desc: '과열 돌진. 전방 전용 — 싸고 강하지만 자신도 화상으로 체력 5를 잃는다.' }),
