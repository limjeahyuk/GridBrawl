...
const ABYSS: CardDef[] = [
  atk({
    id: 'b-abyss-grasp', name: '심연의 손아귀', range: beamBoth(2, 3), damage: 20, energyCost: 18,
    pull: 2, pointBlank: false, fx: 'rush', accent: '#8f5fd4',
    desc: '앞뒤 2~3칸의 상대를 두 칸 끌어당긴다. 물러나 봐야 다시 품 안이다.',
  }),
  atk({
    id: 'b-abyss-tide', name: '파멸의 물결', range: barBoth(1), damage: 26, energyCost: 26,
    drain: 12, fx: 'orb', accent: '#8f5fd4',
    desc: '앞뒤 세로 3줄을 덮는 검은 물결. 적중하면 상대 기력을 12까지 빨아들인다.',
  }),
  atk({
    id: 'b-abyss-brand', name: '심연의 낙인', range: [...CROSS, ...both(2)], damage: 18, energyCost: 20,
    burn: 6, freeze: 1, fx: 'flame', accent: '#8f5fd4',
    desc: '십자 네 칸 + 앞뒤 2칸째에 낙인을 새긴다. 화상 6 + 1턴 빙결.',
  }),
  atk({
    id: 'b-abyss-maw', name: '집어삼키는 아가리', range: [...RING, ...both(2)], damage: 42, energyCost: 46,
    pierce: true, leech: 14, fx: 'quake', signature: true, accent: '#b06fe8',
    desc: '시그니처. 몸을 둘러싼 여덟 칸 + 앞뒤 2칸째를 통째로 삼킨다. 보호막을 무시하고, 입힌 만큼 스스로 아문다.',
  }),
]

// --- 수호기사 · 봉인의 파수꾼 ------------------------------------------------
// 정체성: **때릴 타이밍이 정해져 있다.** 방벽 턴엔 화력을 버리게 만들고, 커튼을
// 여는 턴에 보호막을 통째로 깨며 되갚는다. 플레이어에게 "기다렸다 친다"를 가르친다.
const SANCTUM: CardDef[] = [
  {
    id: 'b-ward-bulwark', name: '봉인의 방벽', kind: 'guard', block: 88, guardCost: 16, cooldown: 0,
  ...
  atk({
    ...
    desc: '앞뒤 세로 3줄을 되받아친다. 휘두르면서 보호막 +16을 두른다.',
  }),
  atk({
    id: 'b-ward-lance', name: '돌파 창격', range: [...bar(1), ...bar(-1)], damage: 32, energyCost: 26,
    dashForward: 2, push: 2, fx: 'rush', accent: '#c9a24a',
    desc: '두 칸 파고들며 창을 내지른다. 맞은 상대는 두 칸 밀린다.',
  }),
  atk({
    id: 'b-ward-verdict', name: '최후의 심판', range: [...barBoth(1), ...both(2)], damage: 44, energyCost: 44,
    shatter: true, stun: 1, fx: 'quake', signature: true, accent: '#e6c05f',
    desc: '시그니처. 앞뒤 세로 3줄 + 앞뒤 2칸째에 심판을 내린다. 보호막을 남김없이 부수고 1턴 기절시킨다.',
  }),
]

// --- 화염군주 · 잿불의 폭군 ---------------------------------------------------
// 정체성: **충전이 실제로 쌓인다.** 예고 턴에 쓰는 「불씨 모으기」가 `empower`로
// 이 전투 내내 남는 공격력이 된다 — 예전엔 "모은다"가 문구뿐이었고 실제로는 아무
// 것도 쌓이지 않았다. 그래서 오래 끌수록 위험해지고, 충전 턴이 곧 반격 창이다.
const PYRE: CardDef[] = [
  atk({
    id: 'b-pyre-ember', name: '불씨 모으기', range: both(1), damage: 12, energyCost: 14,
    empower: 3, fx: 'flame', accent: '#e0663a',
    desc: '앞뒤 한 칸을 툭 지지며 불씨를 모은다. 이 전투 내내 자기 공격 피해 +3(중첩).',
  }),
  atk({
    id: 'b-pyre-lash', name: '화염 채찍', range: beamBoth(1, 3), damage: 24, energyCost: 24,
    burn: 5, fx: 'flame', accent: '#e0663a',
    desc: '같은 줄 앞뒤 3칸까지 채찍처럼 뻗는 불길. 화상 5.',
  }),
  atk({
    id: 'b-pyre-pillar', name: '화염 기둥', range: [...bar(1), ...bar(-1), ...both(2)], damage: 23, energyCost: 30,
    burn: 6, fx: 'flame', accent: '#e0663a',
    desc: '앞뒤 세로 3줄 + 앞뒤 2칸째에서 불기둥이 솟는다. 화상 6.',
  }),
  atk({
    id: 'b-pyre-inferno', name: '인페르노', range: [...barBoth(1), ...barBoth(2)], damage: 32, energyCost: 48,
    burn: 10, fx: 'flame', signature: true, accent: '#ff8a3d',
    desc: '시그니처. 앞뒤 1~2칸의 세 줄을 통째로 불바다로 만든다. 화상 10 — 모아 둔 불씨가 전부 얹힌다.',
  }),
]
