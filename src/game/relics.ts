...
> = {
  warrior: {
    ...
    // ⚠ **AI 예측 개편(2026-08-10)에서 전사가 제일 크게 무너졌다** — 31.6 → 13%.
    //   사거리 1칸이라 붙는 동안 통째로 얻어맞는데, 몬스터가 이제 걸어오는 나를
    //   정확히 조준한다(원거리 hard 몬스터에 특히 약하다 — 팬텀 45.8% · 화염군주
    //   52.7%). 56 → 115로 되돌렸다. 민감도 ≈ **0.16%p/HP**로 셋 중 가장 둔하다.
    runEffect: { damageReduction: 2, regen: 2, thorns: 2, maxHpBonus: 115 },
    desc: '서약의 돌무덤: 최대 체력 +115, 받는 공격 피해 -2, 매 라운드 체력 +2, 피격 시 2 반사.',
  },
  archer: {
    ...
    // ⚠ **민감도가 뒤집혔다(2026-08-10)** — 예전 문서의 "1포인트 ≈ 0.43%p"가 아니라
    //   지금은 **≈0.64%p/HP**로 셋 중 압도적으로 민감하다. 거리를 벌고 싸우는 쪽이라
    //   AI 예측 개편의 타격이 제일 적었고(24.7%), 그래서 18 → 16으로 **내려서** 맞췄다.
    //   전사·마법사와 달리 궁수는 몇 포인트로 밴드가 통째로 움직인다.
    runEffect: { attackBonus: 5, poisonOnHit: 2, damageReduction: 2, maxHpBonus: 16, lifesteal: 2 },
    desc: '독니 화살통: 최대 체력 +16, 받는 피해 -2. 내 공격 피해 +5, 피해를 주면 체력 2 흡수 + 중독 2라운드.',
  },
  mage: {
    ...
    // ⚠ AI 예측 개편(2026-08-10)으로 57 → 75. 민감도 ≈ **0.23%p/HP**.
    runEffect: { turnEnergy: 5, turnShield: 3, burnOnHit: 2, damageReduction: 2, maxHpBonus: 75 },
    desc: '혼백의 등불: 최대 체력 +75, 받는 피해 -2. 매 라운드 기력 +5·보호막 +3, 피해를 주면 화상 2라운드를 묻힌다.',
  },
}
