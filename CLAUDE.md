...
- **덱 빌딩(2026-07-22)**: 전투 전에 **고정 7장 + 고른 7장 = 14장** 덱을 짠다(`src/game/decks.ts`). 고정=이동4방향·스트라이크·브레이스·원기, 선택 풀=대시2·**대각선 이동4(↗↖↘↙)**·펄스샷·가드·**리페어(힐)**+캐릭터 고유4. 덱은 **캐릭터 종속**, localStorage `gb-decks`에 저장(이름/수정/삭제). 모든 덱 카드는 `deckFor(char)`의 부분집합이라 **멀티 플랜 복원(`net/session.ts`)은 그대로 동작**. `PRESET_DECKS`는 봇 상대 덱 + 기본 덱.
