...
### ⏸ 진행 중 — 공격 사거리 앞뒤 대칭화 (2026-08-03, **중단**)

**여기부터 이어서 하면 된다.** 절반만 반영된 상태로 커밋돼 있다 — 전사만 대칭이고 궁수·마법사는 아직 옛날 사거리라 **지금 밸런스는 깨져 있다**(시뮬 미실행).

**왜 하는가** — 겹침·통과·넉백·돌진이 전부 허용되는 격자인데 공격 카드 상당수가 전방 전용이라, 상대를 지나치는 순간 통째로 빗나갔다. 사용자 요구: **공격 카드의 90%를 앞뒤 대칭**(뒤 1칸 · 내 칸 · 앞 1칸이 기본 골격)으로 바꾸고, **한쪽만 노리는 건 "정말 강한 것"에만** 남겨 그게 곧 그 카드의 값이 되게 한다. 설계 규약은 `roster.ts`의 range helper 주석에 이미 써 뒀다.

**끝난 것**
- `roster.ts` — `barBoth(n)` 헬퍼 추가(`bar(n)+bar(-n)`), range 규약 주석 전면 교체
- **전사 4장**: `war-bash` `bar(1)`→`barBoth(1)`·기력 22→26 / `war-quake` 표기만 `barBoth(1)` / `war-oath` 표기만 `barBoth(1)+both(2)` / `war-charge` `bar(1)`→`beamBoth(1,2)`·기력 24→26 (⚠ 2칸 파고드는 카드라 지나쳐 빗나가는 대표 사례였다)

**남은 것 — 아래 표대로 그대로 적용하면 된다**

| 파일 | 카드 | 지금 | 바꿀 것 |
| --- | --- | --- | --- |
| `battle/cards.ts` | `c-jab` 잽 | `[fwd(1)]` | `both(1)` · 기력 8→10 |
...
⚠ `runcards.ts`는 헬퍼를 **자기 파일 안에 따로** 갖고 있다 — `beamBoth`·`barBoth`를 거기에도 추가해야 한다.

**전방 전용으로 남길 예외 6장 (건드리지 말 것)** — 이게 "정말 강한 것"의 값이다:
`arc-venom`(저격·밀착사각) · `arc-rain`(궁수 시그니처·밀착사각) · `r-maul` 42dmg · `r-railgun` 44dmg·밀착사각 · `r-frenzy` 58dmg · `r-annihilate` 66dmg. → 공용+직업 공격 21장 중 **19장(90%) 대칭**, 런 전용까지 합치면 33장 중 27장(82%).

**밀착사각 최종 4장**: `c-shot`(도넛 견제) · `arc-venom` · `arc-rain` · `r-railgun`(+런 전용 `r-hook`·`r-tether`).
⚠ **궁수의 밀착사각을 4장 → 2장으로 줄이는 것이 이 작업 최대의 밸런스 리스크다.** GDD ④·아래 "로스터"에 적혀 있듯 "붙으면 궁수가 죽는다"가 3직업 약점 설계의 핵심인데, 그 약점을 절반 걷어낸다. 시그니처(`arc-rain`)와 저격(`arc-venom`)에는 남겨 정체성은 지켰다.

**UI (미착수)** — `CardFace.RangeChart`의 가운데 칸(`rc--self`)이 밀착 가능 여부를 표시하지 않는다. 공격 카드이고 `pointBlank !== false`면 `rc--pb` 같은 클래스를 얹어 "내 칸도 사거리"라는 걸 보이게 할 것(`ui.css`의 `.rc--self` 옆에 규칙 추가). 사용자가 말한 "유저까지 해서 3칸"이 눈에 보여야 한다.

**문서 (미착수)** — 같은 변경에서 [docs/GAME_DESIGN.md](docs/GAME_DESIGN.md)를 갱신할 것: ③의 **양방향 규약**(153행, 아직 2026-07-14 옛 규약 + 없어진 옛 카드 이름) · ④의 **밀착 타격**(240행, "15장 중 11장 = 73%"·예외 4장 집계가 바뀐다) · 공용 카드 표(89~90·141~142행)의 잽.

**밸런스 — 반드시 다시 잰다.** 아직 **한 번도 안 돌렸다**. 전 카드가 명중률이 올라가는 변경이라 몬스터도 같이 세진다(몬스터가 `c-strike`·`war-bash`·`arc-pin` 등을 그대로 쓴다).
- 변경 전 기준선 (5시드 × 900런 스윕, `--sweep --seed=`): **WARRIOR 27.5 / ARCHER 26.3 / MAGE 24.7 · 밴드 2.8%p** (시드 12345 / 777 / 2024 / 555 / 99)
- 변경 전 PvP (`npm run sim`): 평균 4.71턴 · **ARCHER 85% / WARRIOR 49% / MAGE 17%** — PvP는 원래부터 무너져 있다(위 "남은 것" 2번). 이 작업으로 궁수의 사거리가 3→2로 줄고 마법사 광역이 넓어지니 **오히려 좁혀질 수 있다** — 끝나면 한 번 재 볼 것
...
#### ② 공격 사거리 앞뒤 대칭 전면 적용 (`roster.ts` · **밸런스 미측정**)

겹침·통과·넉백·돌진이 다 허용되는 격자라 "상대를 지나쳐 버리는" 상황이 상시로 생기는데, 전방 전용 카드는 그때마다 통째로 빗나갔다. 그래서 공격의 기본 골격을 **뒤1칸·내칸(밀착)·앞1칸**으로 잡고 넓은 카드는 앞뒤로 늘렸다. 헬퍼 `barBoth(n)` 추가. 전방 전용은 "한 방이 큰" 카드에만 남긴다(궁수 저격 2장 + 런 전용 필살기 4장).

- ⚠ **이 변경은 밸런스를 크게 흔든다** — 사거리가 넓어지면 명중률이 오른다. `npm run sim`(PvP)과 `npm run sim:run -- --sweep`(런) **둘 다 다시 돌려야 한다**. 아직 안 돌렸다.
