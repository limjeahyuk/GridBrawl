...
- **지형 = 바위(2026-08-05)** — 판이 빈 격자뿐이라 어느 전투나 "가로로 붙었다 떨어졌다"만 반복됐다(사용자 요청: "몇몇 맵은 지형이 좀 다르게… 바위를 세워서 그 앞을 뚫고는 지나갈 수 없다던가"). **런 전용**이고 규칙은 셋뿐이다 — ⓐ **못 들어간다**(이동·이동공격·넉백·끌어당김이 벽과 같은 자리에서 멈춘다) ⓑ **사격선을 끊는다**(같은 줄·같은 열일 때만. 대각은 안 가리므로 **줄을 옮기는 게 늘 우회로**다) ⓒ **부술 수 있다**(체력 34~62. 공격이 덮은 칸의 바위 + **그 공격을 가로막은** 바위가 깎인다 → 사거리가 짧아도 결국 뚫는다). 상세 GDD ④ "지형 — 바위"
  ...
  - **처박기 기절** — 밀어낼 곳이 바위라 **한 칸도 못 밀리면 1턴 기절**. ⚠ **벽은 해당 없다**(가장자리는 늘 거기 있어서 포함하면 "구석에 몰린 상대를 매 턴 기절"이 된다 — 지형을 만든 대가가 아니라 좌석 운이다)
...
- **지속 상태이상(2026-08-01)** — 독·화상·빙결. 3직업 개편의 1단계로 엔진에 먼저 넣었고, **2단계에서 카드에 연결됐다**(궁수=독 / 마법사=화상·빙결, 위 "로스터" 참고). 그래서 **더 이상 밸런스 중립이 아니다** — 상태이상 수치를 건드리면 `npm run sim:run -- --sweep`을 다시 돌려야 한다. 전장 붕괴와 같은 자리(턴 종료·보호막 무시·랜덤 없음)에서 돌아 멀티 락스텝 안전. 타입 `types.ts`(`StatusKind/StatusEffect/STATUS_TURNS`), 정산 `engine.resolveTurn`의 `tickStatuses`, 유물 훅 `poisonOnHit/burnOnHit/statusPowerPct/bonusVsAfflicted`. 검증 `npm run check`.
  ...
  - ⚠ **빙결은 UI에서도 이동을 막아야 한다(2026-08-05 수정)** — 엔진만 막고 화면은 아무 말이 없어서, 얼어 있어도 **갈 수 있는 칸이 노랗게 밝혀지고** 눌러서 슬롯에 담기기까지 했다. 실행하면 조용히 무효 처리되니 "넉백 당하고 나서 이동이 안 되는 버그"로 신고됐다(실제로 몬스터 중 `war-bash`로 밀고 `mag-frost`로 얼리는 조합이 있다). 이제 `selectable()`이 빙결 중 이동 카드를 거르고 — 칸 강조·이동 칩·`addCard`가 전부 이걸 통과하므로 **한 곳만 고치면 된다** — 화살표 자리에는 이유(`❄ 얼어붙음 · 이번 턴 이동 불가`)를 놓는다. **흐린 화살표만 남기면 "왜 안 눌리지"가 되어 신고가 반복된다.**
- **지형 — 바위(2026-08-05)**: 판에 **막힌 칸**을 만든다. **못 들어가고**(이동·`dashForward`·넉백·끌어당김이 벽과 같은 자리에서 멈춤) **사격선을 끊고**(같은 줄·같은 열일 때만 — 대각은 안 가려 줄 바꾸기가 늘 우회로) **부술 수 있다**(체력 34~62). **관통(`pierce`·유물 `alwaysPierce`)은 무시하고 뚫는다**. 밀어낼 곳이 바위라 한 칸도 못 밀리면 **1턴 기절**(벽은 해당 없음). 타입·판정 `types.ts`(`Obstacle`/`ROCK_HP`/`rockAt`/`canStand`/`shadowRock`/`RockPlan`), 적용 `engine.resolveTurn`, 배치 `run.ts`의 `SCENE_TERRAIN`. **런 전용**(`BattleOpts.obstacles` — PvP·봇전은 빈 판이라 규칙이 no-op) → `RULES_VERSION` 안 올림. 상세 GDD ④ "지형 — 바위".
