...
**문서(미착수)** — 같은 커밋에서 [docs/GAME_DESIGN.md](docs/GAME_DESIGN.md)를 갱신할 것: ③ **양방향 규약**(153행 — 아직 2026-07-14 옛 규약이고 없어진 옛 카드 이름을 든다) · ④ **밀착 타격**(240행 — "15장 중 11장 = 73%"와 예외 4장 집계가 바뀐다) · 공용 카드 표(89~90·141~142행)의 잽.

**밸런스 — 반드시 다시 잰다. 아직 한 번도 안 돌렸다.** 전 카드의 명중률이 오르는 변경이고, 몬스터도 같은 카드를 쓰므로(`c-strike`·`war-bash`·`arc-pin` 등) 같이 세진다.
- 변경 전 기준선(5시드 × 900런 `--sweep`, 시드 12345/777/2024/555/99): **WARRIOR 27.5 / ARCHER 26.3 / MAGE 24.7 · 밴드 2.8%p**
- 변경 전 PvP(`npm run sim`): 평균 **4.71턴** · ARCHER 85% / WARRIOR 49% / MAGE 17% — PvP는 원래부터 무너져 있다(위 "남은 것" 2번). 이번 변경은 궁수 사거리를 3→2로 줄이고 마법사 광역을 넓히니 **오히려 좁혀질 수 있다** — 끝나면 한 번 재 볼 것
