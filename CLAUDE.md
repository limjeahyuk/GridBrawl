...
#### ①-bis 초상은 왼쪽, 전투는 상대 쪽 (2026-08-03)

방향은 **두 상황이 다르다**. 한쪽 규칙으로 뭉뚱그리면 반드시 어색해진다.

| 화면 | 기준 | 어떻게 |
| --- | --- | --- |
| 메뉴·초상 (상대가 없다) | **왼쪽 고정** | `PortraitSvg`가 `facesRight` 시트만 뒤집어 왼쪽으로 통일 |
| 전투 (상대가 있다) | **상대 쪽** | `faceToward`가 매 렌더 열을 비교 |

- 왼쪽으로 잡은 이유: 시트 원본이 왼쪽을 보고 그려진 전사를 억지로 뒤집으면 **방패와 검이 반대 손으로 가서** 그 캐릭터만 어색해진다. 셋이 제각각 보면 목록도 어수선하다.
- ⚠ **필살기 컷인도 `PortraitSvg`를 쓴다** — 그전엔 `buildPortraitSvg`(옛 절차 SVG)를 직접 불러서, 필살기가 터지면 화면 전체가 **옛 사이버 로봇 아트**로 덮였다. 이제 `buildPortraitSvg`의 유일한 호출처는 `PortraitSvg`의 SVG 폴백뿐이다(시트 없는 캐릭터용).
