...
- **벽 격돌 + 상태이상 HUD 줄(2026-08-08)** — 둘 다 **연출·표시뿐이고 룰은 안 건드렸다**(밸런스 재측정 결과 수치 불변). 넉백이 판 끝에 막히면 그 벽에서 격돌이 터지고, 지속효과 칩이 기력 바 아래에도 뜬다. 상세·함정은 아래 "전투 연출 / 손맛"
...
- **전투 연출 / 손맛(2026-07-31)** — 룰에는 영향이 없고 **연출 레이어만** 손댄다(엔진·AI·시뮬 불변).
  ...
  - **벽 격돌(2026-08-08)** — 넉백·끌어당김이 **판 끝에 막히면** 부딪힌 쪽 가장자리에서 균열 섬광 + 돌먼지가 터지고, 맞은 파이터가 그 방향으로 처박혔다 튕겨 나온다(`fighter--sprite.is-slam` / `board__slam`, 소리 `slam`, 카메라 펀치 3 + 히트스톱 +60ms). 그전엔 **벽에 몰린 것과 한 칸 밀려난 것이 화면에서 똑같아** 넉백 카드가 벽을 등진 상대에게 제일 아프다는 게 안 드러났다.
    - 근거는 엔진이 새로 다는 **`Step.slam`**(±1, 정규 좌표) — `applyShove`가 판 밖으로 나가려다 멈추면 밀려나던 방향을 돌려주고 `emit`이 스텝에 실어 준다. ⚠ **연출 주석이지 룰이 아니다**: 격돌해도 추가 피해도 기절도 없고 판 상태는 이 값이 없을 때와 완전히 같다. 그래서 `RULES_VERSION`도 **안 올린다**(락스텝은 상태가 같으면 성립). 5시드 스윕 재측정도 **수치가 한 자리도 안 움직였다**.
    ...
    - 검증 `npm run check` 9건(막혔을 때만 붙는가 · 방향 부호 · 이미 벽이어도 격돌 · 피해/기절 불변).
